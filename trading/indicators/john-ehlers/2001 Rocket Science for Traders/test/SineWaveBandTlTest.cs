...
{
    [TestClass]
    public class SineWaveBandTlTest
    {
        #region Test data
        /// <summary>
        ...
        {
            var target = new SineWaveBandTl();
            Assert.AreEqual("SineBand", target.Name);
        }
        #endregion
        ...
        {
            var target = new SineWaveBandTl();
            Assert.AreEqual("SineBand4", target.Moniker);
            //target = new SineWaveBandTl(4);
            //Assert.AreEqual("SineBand4", target.Moniker);
            target = new SineWaveBandTl(3);
            Assert.AreEqual("SineBand3", target.Moniker);
            target = new SineWaveBandTl(2);
            Assert.AreEqual("SineBand2", target.Moniker);
        }
        #endregion
        ...
        {
            var target = new SineWaveBandTl();
            Assert.AreEqual("Sine Wave / Sine Wave Lead Band", target.Description);
        }
        #endregion
        ...
        public void DominantCyclePeriodTest()
        {
            ...
            for (int i = 9; i < rawInput.Count; ++i)
            {
                ...
                target.Update(rawInput[i], dateTime);
        ...
        public void ResetTest()
        {
            DateTime dateTime = DateTime.Now;
            var target = new SineWaveBandTl();
            var updatedList1 = rawInput.Select(t => target.Update(t, dateTime)).ToList();
            target.Reset();
            var updatedList2 = rawInput.Select(t => target.Update(t, dateTime)).ToList();
            for (int i = 0; i < rawInput.Count; ++i)
            {
                Assert.AreEqual(updatedList1[i].FirstValue, updatedList2[i].FirstValue);
                Assert.AreEqual(updatedList1[i].SecondValue, updatedList2[i].SecondValue);
                Assert.AreEqual(updatedList1[i].Time, updatedList2[i].Time);
            }
        }
        #endregion
        ...
        /// </summary>
        [TestMethod]
        public void SineWaveBandTlConstructorTest()
        {
            var target = new SineWaveBandTl();
            Assert.AreEqual(4, target.SmoothingLength);
            Assert.IsFalse(target.IsPrimed);
            ...
            target = new SineWaveBandTl(OhlcvComponent.TypicalPrice);
            Assert.AreEqual(4, target.SmoothingLength);
            Assert.IsFalse(target.IsPrimed);
            ...
            target = new SineWaveBandTl(3, OhlcvComponent.OpeningPrice);
            Assert.AreEqual(3, target.SmoothingLength);
            Assert.IsFalse(target.IsPrimed);
            ...
            target = new SineWaveBandTl(2, OhlcvComponent.OpeningPrice);
            Assert.AreEqual(2, target.SmoothingLength);
            Assert.IsFalse(target.IsPrimed);
            Assert.IsTrue(target.OhlcvComponent == OhlcvComponent.OpeningPrice);
        }
        ...
        public void SineWaveBandTlConstructorTest2()
        {
            var target = new SineWaveBandTl(5);
            Assert.IsFalse(target.IsPrimed);
        }
        ...
        [TestMethod]
        [ExpectedException(typeof(ArgumentOutOfRangeException))]
        public void SineWaveBandTlConstructorTest3()
        {
            var target = new SineWaveBandTl(1);
            Assert.IsFalse(target.IsPrimed);
        }
        ...
        [TestMethod]
        [ExpectedException(typeof(ArgumentOutOfRangeException))]
        public void SineWaveBandTlConstructorTest4()
        {
            var target = new SineWaveBandTl(0);
            Assert.IsFalse(target.IsPrimed);
        }
        ...
        [TestMethod]
        [ExpectedException(typeof(ArgumentOutOfRangeException))]
        public void SineWaveBandTlConstructorTest5()
        {
            var target = new SineWaveBandTl(-8);
            Assert.IsFalse(target.IsPrimed);
        }
        #endregion

        #region SerializationTest
        private static void SerializeTo(SineWaveBandTl instance, string fileName)
        {
            var dcs = new DataContractSerializer(typeof(SineWaveBandTl), null, 65536, false, true, null);
        ...
        private static SineWaveBandTl DeserializeFrom(string fileName)
        {
            var fs = new FileStream(fileName, FileMode.Open);
            XmlDictionaryReader reader = XmlDictionaryReader.CreateTextReader(fs, new XmlDictionaryReaderQuotas());
            var ser = new DataContractSerializer(typeof(SineWaveBandTl), null, 65536, false, true, null);
            var instance = (SineWaveBandTl)ser.ReadObject(reader, true);
            reader.Close();
            fs.Close();
            return instance;
        }
        ...
        public void SerializationTest()
        {
            ...
            var source = new SineWaveBandTl();
            // Tradestation implementation skips first 9 bars.
            for (int i = 9; i < 99; i++)
            {
                band = source.Update(rawInput[i], dateTime);
                d = Math.Round(source.DominantCyclePhase, digits);
                u = Math.Round(dcPhase[i], digits);
                if (!double.IsNaN(d))
                    Assert.AreEqual(u, d);
                d = Math.Round(source.DominantCyclePeriod, digits);
                u = Math.Round(dcPeriod[i], digits);
                if (!double.IsNaN(d))
                    Assert.AreEqual(u, d);
                d = Math.Round(source.SineWave, digits);
                u = Math.Round(sine[i], digits);
                if (!double.IsNaN(d))
                    Assert.AreEqual(u, d);
                d = Math.Round(source.SineWaveLead, digits);
                u = Math.Round(sineLead[i], digits);
                if (!double.IsNaN(d))
                    Assert.AreEqual(u, d);
            ...
            const string fileName = "SineWaveBandTl_1.xml";
            SerializeTo(source, fileName);
            SineWaveBandTl target = DeserializeFrom(fileName);
            Assert.AreEqual(source.SineWave, target.SineWave);
            Assert.AreEqual(source.SineWaveLead, target.SineWaveLead);
            Assert.AreEqual(source.DominantCyclePhase, target.DominantCyclePhase);
            ...
            for (int i = 99; i < rawInput.Count; i++)
            {
                ...
                d = Math.Round(target.SineWave, digits);
                u = Math.Round(target.SineWave, digits);
                if (!double.IsNaN(d))
                    Assert.AreEqual(u, d);
                d = Math.Round(target.SineWaveLead, digits);
                u = Math.Round(target.SineWaveLead, digits);
                if (!double.IsNaN(d))
                    Assert.AreEqual(u, d);
                d = Math.Round(target.DominantCyclePhase, digits-1);
                u = Math.Round(dcPhase[i], digits-1);
                if (!double.IsNaN(d))
                    Assert.AreEqual(u, d);
                d = Math.Round(target.DominantCyclePeriod, digits);
                u = Math.Round(dcPeriod[i], digits);
                if (!double.IsNaN(d))
                    Assert.AreEqual(u, d);
                d = Math.Round(band.FirstValue, digits);
                u = Math.Round(band.FirstValue, digits);
                if (!double.IsNaN(d))
                    Assert.AreEqual(u, d);
                d = Math.Round(band.SecondValue, digits);
                u = Math.Round(band.SecondValue, digits);
                if (!double.IsNaN(d))
                    Assert.AreEqual(u, d);
                Assert.AreEqual(dateTime, band.Time);
            }
