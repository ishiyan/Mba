...
int init() {
    drawBegin = 4;
    initBuffer(Filter, "Two-Pole Butterworth Filter", DRAW_LINE);
    IndicatorBuffers(buffers);
    IndicatorShortName("Two-Pole Butterworth Filter [" + CutoffPeriod + "]");  
...
int start() {
    ...
    for (s = limit; s >= 0; s--) {
        Filter[s] = coef1 * (P(s) + 2.0 * P(s + 1) + P(s + 2)) +
                    coef2 * Filter[s + 1] + 
                    coef3 * Filter[s + 2];
        if (s > Bars - 4) {
            Filter[s] = P(s);
