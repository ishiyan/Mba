...
//----
double _CCI(int index,double price[],int shift)
{
   double mean = 0;
   for(int j=0;j<index+StartLength;j++) mean += price[j];
   
   mean = mean/(index + StartLength);
       
   double avgDev = 0 ;
   for(j=0;j<index+StartLength;j++) avgDev += MathAbs(price[j] - mean);
   
   avgDev = avgDev/(index+StartLength);

   if(avgDev > 0) double cci = (price[0] - mean)/(0.015*avgDev); else cci = 0;
...
double EMA(int index,double price,int per,int bar)
{
   ...
   ema[index][0] = ema[index][1] + alpha*(price - ema[index][1]); 
