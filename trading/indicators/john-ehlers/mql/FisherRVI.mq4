...
//|                                                    FisherRVI.mq4 |
//|                                                                  |
//| Fisher Stochastic Relative Vigor Index Indicator                 |
//|                                                                  |
...
int init() {
    drawBegin = Length;
    initBuffer(FisherRVI, "Fisher RVI", DRAW_LINE);
    ...
    IndicatorShortName("Fisher RVI [" + Length + "]");
    return (0);
}
...
int start() {
    ...
    for (s = limit; s >= 0; s--) {
        ...
        Value3[s] = 0.0;
        if (hh != ll) {
            Value3[s] = (RVI[s] - ll) / (hh - ll);
        }
        Value4[s] = (4.0 * Value3[s] + 3.0 * Value3[s + 1] + 2.0 * Value3[s + 2] + Value3[s + 3]) / 10.0;
        FisherRVI[s] = 0.5 * MathLog((1.0 + 1.98 * (Value4[s] - 0.5)) / (1.0 - 1.98 * (Value4[s] - 0.5)));
        Trigger[s] = FisherRVI[s + 1];
    }
    return (0);   
}
