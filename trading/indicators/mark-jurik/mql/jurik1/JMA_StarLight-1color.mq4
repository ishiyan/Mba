...
//---- indicator buffers 
double JMA_Buf[ ], JMA_Buf_Up[], JMA_Buf_Down[]; 
double MEMORY1[9];
int    MEMORY2[12];
double MEMORY3[128]; 
...
SetIndexBuffer (1, JMA_Buf_Down); 
SetIndexEmptyValue(1,0.0); 
SetIndexBuffer (2, JMA_Buf_Up); 
...
//+GGGGGGGGGGGGGGGGG GGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGG+ 
...
if(JMA_Buf[bar] > JMA_Buf[bar+1]){
 JMA_Buf_Up[bar] = JMA;
 JMA_Buf_Down[bar] = 0;
}else if(JMA_Buf[bar] < JMA_Buf[bar+1]){
 JMA_Buf_Down[bar] = JMA;
 JMA_Buf_Up[bar] = 0;
}else{
 JMA_Buf_Down[bar] = 0;
 JMA_Buf_Up[bar] = 0;
